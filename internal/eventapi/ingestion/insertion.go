...
func InsertEvents(ctx context.Context, db *eventdb.RedisEventStore, msgs chan *model.BatchUpdate, bufferSize int) chan []*pulsarutils.ConsumerMessageId {
...
func insert(db *eventdb.RedisEventStore, rows []*model.Event) {
	start := time.Now()
